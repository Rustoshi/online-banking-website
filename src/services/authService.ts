...
export class AuthService {
   ...
   * Register a new user
   */
  static async register(data: RegisterInput): Promise<{ user: Partial<IUser>; token: string; refreshToken: string }> {
  ...
  static async login(data: LoginInput, ipAddress?: string): Promise<LoginResponse> {
    ...
    if (user.status === UserStatus.DORMANT) {
      throw new Error('Your account is dormant. Please contact support to reactivate.');
    }
